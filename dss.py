...
║  FREE: 1 ringkasan gabung semua pair                   ║
║  VIP: 1 ringkasan + 1 pesan per pair (full detail)     ║
║  Duplicate guard per siklus                            ║
╚══════════════════════════════════════════════════════════╝
"""
...
# DISTRIBUTION STATE
# ============================================================
CYCLE_SENT_CACHE = set()
FREE_SENT_FLAG = False
...
def scoring_engine(structure_data, trend_data, momentum_data,
    ...
    s = structure_data["score"]; t = trend_data["score"]; m = momentum_data["score"]
    v = vol_data["score"]; l = liq_data["score"]; f = flow_data["score"]; q = squeeze_data["score"]
    ...
    if total >= SCORE_THRESHOLD:
        ...
        if direction != "NETRAL": return direction, audit
...
def risk_engine(symbol, signal, candles_15m):
    ...
    return {"entry": entry, "stop_loss": stop_loss, "take_profit_1": take_profit_1,
            "take_profit_2": take_profit_2, "risk_reward": rr, "atr_15m": atr_15m}
...
def run_analysis_engine(cycle_count):
    global CYCLE_SENT_CACHE, FREE_SENT_FLAG
    CYCLE_SENT_CACHE = set()
    FREE_SENT_FLAG = False
...
# ============================================================
# DISTRIBUTION (FINAL — SESUAI RULE)
# ============================================================
def free_distribution(signals, btc_regime):
    global FREE_SENT_FLAG
    if FREE_SENT_FLAG: return
    if signals is None: signals = []
    send_to_telegram(TELEGRAM_FREE_ID, format_summary(signals, btc_regime, "FREE"))
    FREE_SENT_FLAG = True

def vip_distribution(signals, btc_regime):
    global CYCLE_SENT_CACHE
    if signals is None: signals = []
    send_to_telegram(TELEGRAM_VIP_ID, format_summary(signals, btc_regime, "VIP"))
    for s in signals:
        key = f"{s['symbol']}_{s['signal']}"
        if key in CYCLE_SENT_CACHE: continue
        send_to_telegram(TELEGRAM_VIP_ID, format_signal_vip(s))
        CYCLE_SENT_CACHE.add(key)
...
def main():
    get_session()
    print("="*60)
    print("DSS MARKET v8 — FINAL DISTRIBUTION LOGIC")
    print(f"Siklus: {SIKLUS_DETIK//60} menit | Retention: {RETENTION_DAYS} hari")
    print(f"FREE: 1 ringkasan | VIP: ringkasan + full per pair")
    print("="*60)
    cycle = 0
    while True:
        cycle += 1
